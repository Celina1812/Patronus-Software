...
<head>
	...
	<style>
		...
		.circuit-container {
			...
			/* Espaciado uniforme entre pasos */
			padding: 20px 0;
			margin-top: 20px;
		}
		...
		.step {
			text-align: center;
			width: 100px;
			/* Ajusta ancho a tamaño específico */
			padding: 10px;
			border: 2px dashed #ccc;
			/* Estilo de borde actualizado */
			border-radius: 10px;
			cursor: pointer;
			...
			<div class="main-content">
				<div class="wrap-content container" id="container">
					<section id="page-title">
						<div class="row">
							<div class="col-sm-8">
								<!-- Incluye el resto de tu estructura -->
								<div class="container">
		...
		<script>
			function selectLocation(location) {
				...
				document.querySelectorAll('.step').forEach(step => {
					step.style.border = '2px dashed #ccc'; // Restablece el borde
				});

				// Marcar el icono seleccionado
				const selectedStep = document.getElementById(location);
				if (selectedStep) {
					selectedStep.style.border = '2px solid blue'; // Cambia el borde del icono seleccionado
				}

				// Enviar la ubicación al servidor usando AJAX
				const equipoId = <?php echo json_encode($codigo_equipo); ?>; // Cambiar 'equipo_id' por 'codigo_equipo'

				$.ajax({
					url: '', // La misma página PHP
					type: 'POST',
					data: {
						ubicacion: location,
						equipo_id: equipoId // Este valor ahora es el codigo_equipo
					},
